...
m1 <- clmm(rating ~ sentence_type_pol + (1 | participant_no),  
           data = polish_data, Hess=TRUE) 
...
clmm.graph <- clmm(rating ~ sentence_type_pol + (1 | participant_no),  
                   data = polish_data, Hess=TRUE) 
...
vlines <- c(0, # intercept, 0 = sentence_type_noracceptable_di_refl
            ...
            tibble(xaxis,yaxis) %>% # baseline tibble for plot dimensions
              ...
              ggsave(here("plots", "polish_prob_curve.png"), dpi = 900)
