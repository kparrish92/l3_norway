# Run a frequentist model and create probability curve plots based on 
# work by Lauren Ackerman (https://verbingnouns.github.io/notebooks/ordinal_data.html)
...
m1 <- clmm(rating ~ sentence_type_eng + (1 | participant_no),  
           data = english_data, Hess=TRUE) 
...
clmm.graph <- clmm(rating ~ sentence_type_eng + (1 | participant_no),  
                   data = english_data, Hess=TRUE) 
...
tibble(xaxis,yaxis) %>% # baseline tibble for plot dimensions
  ...
  ggsave(here("plots", "english_prob_curve.png"), dpi = 900)